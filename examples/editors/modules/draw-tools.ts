...
export function activate(context: ModuleContext): void {
  ...
  context.services.register(TOOLSET_CONTROL, control, { properties: { bundle: 'draw-tools' } })
  ...
  undo?.push('draw-tools started')
  context.services.get<Journal>(JOURNAL)?.note(
    `draw-tools: undo stack has ${undo?.entries.length ?? 0} entry`
  )
}
...
function restore(): void {
  ...
  host.services.get<Journal>(JOURNAL)?.note('draw-tools: 2 tools registered')
}
...
function withdraw(): void {
  ...
  host?.services.get<Journal>(JOURNAL)?.note('draw-tools: tools withdrawn')
}
